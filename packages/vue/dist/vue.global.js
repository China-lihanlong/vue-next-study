var Vue = (function (exports) {
  ...
  */
  ...
  function reactive(target) {
      ...
      return createReactiveObject(target, // 原始数据
      false, // 数据是否可变
      mutableHandlers, // 可变数据代理劫持方法
      mutableCollectionHandlers, // 可变集合数据代理劫持方法
      reactiveMap // 代理对象映射表
      );
  }
  /**
  ...
  // 判断是不是响应式对象 如果是只读需要拿到原始数据再次递归判断
  ...
  function renderComponentRoot(instance) {
      ...
      try {
          if (vnode.shapeFlag & 4 /* STATEFUL_COMPONENT */) {
              ...
              // render 是一定是得到了 如果用户没有设置 编译器会进行编译后进行赋值 最后再把render函数执行返回结果返回
  ...
  */
  ...
  function createAppAPI(render, hydrate) {
      return function createApp(rootComponent, rootProps = null) {
          ...
          const app = (context.app = {
              ...
              // Vue2的静态方法不复存在了 全部变成了实例方法 也就是由 Vue.xxx 变成了 app.xxx 这样可以非常好的利用摇树优化 不会出现 dead code 使用就打包 不使用就不打包 Vue3 中的 filter 以及被移除了
              mixin(mixin) {
                  {
              ...
              mount(rootContainer, isHydrate, isSVG) {
                  if (!isMounted) {
                      // 创建一个根组件 将他变成虚拟DOM
                      // 在 虚拟DOM 中 如果是自定义组件由组建的名称变成了组件的配置对象 而存在， 标签依旧是字符串
  ...
  function baseCreateRenderer(options, createHydrationFns) {
      ...
      const patchElement = (n1, n2, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
          ...
          let vnodeHook;
          // 子元素是一个组件 beforeUpdate onVnodeBeforeUpdate是它的生命周期函数
      ...
      // 调用patch去对比每一对比Fragment中的元素，找到父元素的(也可以指定)，
      ...
      const processFragment = (n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, slotScopeIds, optimized) => {
          ...
          // patchFlag：那些是动态的 dynamicChildren：动态的子节点 slotScopeIds：作用域插槽的索引
      ...
      const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => {
          const componentUpdateFn = () => {
              if (!instance.isMounted) {
                  ...
                  // updateComponent 组件更新 可能是内部数据发生了变化(next为null)，也可能是父组件发生变化引发子组件diff(next为VNode)
          ...
          () => queueJob(instance.update), instance.scope // track it in component's effect scope
          );
  ...
  // 先初始化一个公共的VNode，然后再通过外部传入的属性，进行修改
  ...
  function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
      ...
      if (isVNode(type)) {
          // 在传递虚拟VNode，可能被对其的引用会丢失，希望能合并引用
      ...
      // class component normalization. 标记为类组件
      if (isClassComponent(type)) {
          type = type.__vccOpts;
      }
      // class & style normalization. 标准化 props 与 style
      if (props) {
          // for reactive or proxy objects, we need to clone it to enable mutation.
          // 如果是响应式对象就不做处理直接返回 不是克隆一份进行操作
  ...
  function renderList(source, renderItem, cache, index) {
      ...
      if (isArray(source) || isString(source)) {
          ...
          // Number 是无法遍历和迭代的 警告 但是默认返回一个空数组 避免报错
          ...
          if (source[Symbol.iterator]) {
              // 带有迭代器的对象 如：Map WeakMap Set WeakSet 也可以是带有自定义迭代器的
  ...
  // 在通过唯一代理接口，开始向数据对象setupState获取和修改数据
  const PublicInstanceProxyHandlers = {
      get({ _: instance }, key) {
          ...
          // 是 <script setup> 写法 可以直接从数据对象获取数据 这种写法的优先级最高
          ...
          // hasOwn() 是 Object.prototype.hasOwnProperty
          // 获取数据之前需要调用 借助hasOwn() 确认数据的来源 
          // accessCache 是对每一个访问的属性进行分类缓存，保证hasOwn()不需要多次调用，在下一次访问的时候判断缓存就知道去哪拿到数据放回给代理为外界了
          // 缓存的key就是访问的属性，value就是key所属的数据类别，比如 setup、context、props
          // 但是第一次访问的时候，依旧需要调用一次 hasOwn()
          let normalizedProps;
          // 访问不以 '$' 开头的属性 
          if (key[0] !== '$') {
          ...
          (cssModule = type.__cssModules) &&
              ...
              // user may set custom properties to `this` that start with `$` // 用户自定义 '$'开头公共方法
              accessCache[key] = 4 /* CONTEXT */;
              return ctx[key];
          }
          else if (
          // global properties // 上下文配置中的全局属性
      ...
      set({ _: instance }, key, value) {
          ...
          // data、setupState、context中的全局属性、自由属性是可以被修改的
          // props $**** 是只读 不可以被修改的 
  ...
  const RuntimeCompiledPublicInstanceProxyHandlers = /*#__PURE__*/ extend({}, PublicInstanceProxyHandlers, {
      get(target, key) {
          // fast path for unscopables when using `with` block 如果是被排除在外的数据无法通过代理
  ...
  function doWatch(source, cb, { immediate, deep, flush, onTrack, onTrigger } = EMPTY_OBJ) {
      ...
      let getter; // 主要是用来获取数据(在获取数据过程中收集依赖)
  ...
  const createApp = ((...args) => {
      ...
      app.mount = (containerOrSelector) => {
          // 获取挂载容器 找不到容器直接退出 获取容器的方法是 querySelector()
  ...
  // ancestors 存储的是parentNode 我希望每次都可以拿到离当前元素最近的parentNode 最外层节点遍历完毕后清空
  function parseChildren(context, mode, ancestors) {
              ...
              if (!context.inVPre && startsWith(s, context.options.delimiters[0])) {
                  ...
                  if (s.length === 1) {
                      ...
                      if (startsWith(s, '<!--')) {
                          ...
                          // CDATA 标记 (在标记CDATA下，所有的标记、实体引用都被忽略，而被XML处理程序[只能在XML中使用]一视同仁地当做字符数据看待，CDATA的形式如下 <![CDATA[文本内容]]>)
                          if (ns !== 0 /* HTML */) {
      ...
      // Whitespace handling strategy like v2  v2 中的空白处理
  ...
  function isComponent(tag, props, context) {
      ...
      if (
      // <component/> <transition/> <transitionGrop/> <keep-alive/>
      tag === 'component' ||
          /^[A-Z]/.test(tag) ||
          isCoreComponent(tag) ||
  ...
  function parseInterpolation(context, mode) {
      ...
      // 返回的对象：loc记录的是字符串的位置 以及字符串的内容 content包含数据的名称以及名称在字符串中的位置以及当前节点详细信息
      return {
  ...
  // 提供给parseChildren用来获取离当前元素最近的外层元素
  function last(xs) {
